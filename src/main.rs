...
fn main() -> Result<()> {
    ...
    println!("Actions: {} nav, {} arm, {} head, {} waist", nav_count, arm_count, head_count, waist_count);
...
fn load_log_lines(log_path: &str) -> Result<Vec<LogLine>> {
    ...
    // 支持两种时间戳格式：旧格式 [timestamp] 和新格式 [INFO] [timestamp]
...
fn detect_rounds(lines: &[LogLine], t_last: f64) -> Result<Vec<Round>> {
    let type0_regex = Regex::new(r"\[master_control\]:\s*物体类型:\s*0")?;
    let pose_regex = Regex::new(r"\[master_control\]:\s*姿态字符串:\s*(\{.*\})")?;
    let stop_regex = Regex::new(r"\[DynamicTaskManager\]:\s*停止任务:")?;
    let finish_regex = Regex::new(r"\[master_control\]:\s*任务控制完成:")?;
    
    let mut rounds = Vec::new();
    let mut current: Option<Round> = None;
    
    for line in lines {
        if type0_regex.is_match(&line.line) {
        ...
        if let Some(ref mut round) = current {
            ...
            if round.end_ts.is_none() {
                if stop_regex.is_match(&line.line) || finish_regex.is_match(&line.line) {
                    round.end_ts = Some(line.timestamp);
                }
            }
        }
...
fn detect_flows(lines: &[LogLine], rounds: &[Round]) -> Result<Vec<NavigationFlow>> {
    ...
    let nav_end_regex = Regex::new(r"\[导航\]:\s*导航完成，结果代码:\s*0")?;
    
    // 机械臂相关正则 - 更灵活的匹配
    let arm_set_regex = Regex::new(r"\[DoubleArmAction\]:?\s*.*setGoal.*action_type_code:\s*(\d+)")?;
    let arm_ok_regex = Regex::new(r"\[DoubleArmAction\]:?\s*([^:]+):\s*onResultReceived.*Status\s*=\s*(\d+)")?;
    
    // 头部控制相关正则
    let head_start_regex = Regex::new(r"\[头部控制\]:\s*HeadControlAction2\[head_control\]\s*-\s*开始执行")?;
    let head_end_regex = Regex::new(r"\[头部控制\]:\s*HeadControlAction2\[head_control\]\s*-\s*头部已到达目标位置")?;
    
    // 腰部控制相关正则  
    let waist_start_regex = Regex::new(r"\[腰部\]:\s*WaistAction2\[WaistAction2\]\s*-\s*开始执行")?;
    let waist_end_regex = Regex::new(r"\[腰部\]:\s*腰部动作完成，成功:\s*1")?;
    
    let mut flows = Vec::new();
    let mut current_flow: Option<NavigationFlow> = None;
    let mut pending_action: Option<ActionOperation> = None;
    ...
    for line in lines {
        ...
        // 机械臂动作设置
        if let Some(caps) = arm_set_regex.captures(&line.line) {
            // 处理之前的pending action
            if let Some(action) = pending_action.take() {
                // 尝试添加到当前流程，如果没有当前流程则添加到最后一个已完成的流程
                if let Some(ref mut flow) = current_flow {
                    flow.operations.push(ActionOperation {
            ...
            // 创建新的pending action
            if current_flow.is_some() || !flows.is_empty() {
                pending_action = Some(ActionOperation {
                    action_type: "arm".to_string(),
                    action_code: caps[1].parse().ok(),
        ...
        // 机械臂动作完成
        if let Some(caps) = arm_ok_regex.captures(&line.line) {
            let label = caps[1].trim().to_string();
            let status = caps[2].trim();
            
            if let Some(action) = pending_action.take() {
                if action.action_type == "arm" {
                    // 优先添加到当前流程
                    if let Some(ref mut flow) = current_flow {
                        flow.operations.push(ActionOperation {
                            action_type: "arm".to_string(),
                            ...
                            status: if status == "0" { "ok".to_string() } else { format!("status_{}", status) },
                        });
                    } else if let Some(flow) = flows.last_mut() {
                        flow.operations.push(ActionOperation {
                            action_type: "arm".to_string(),
                ...
                if let Some(ref mut flow) = current_flow {
                    flow.operations.push(ActionOperation {
                        ...
                        start_ts: None,
                        end_ts: Some(line.timestamp),
                        status: if status == "0" { "orphan_ok".to_string() } else { format!("orphan_status_{}", status) },
                    });
                } else if let Some(flow) = flows.last_mut() {
                    flow.operations.push(ActionOperation {
                        action_type: "arm".to_string(),
...
fn build_csv_records(flows: &[NavigationFlow], rounds: &[Round], t0: f64) -> Vec<CsvRecord> {
    ...
    for (flow_id, flow) in flows.iter().enumerate() {
        ...
        records.push(CsvRecord {
            ...
            round_start_rel_s: round_info.map(|r| (r.start_ts - t0) * 1000.0).map(|x| (x as u64) as f64 / 1000.0),
            round_end_rel_s: round_info.and_then(|r| r.end_ts).map(|ts| (ts - t0) * 1000.0).map(|x| (x as u64) as f64 / 1000.0),
            round_duration_s: round_info.and_then(|r| r.end_ts).map(|end_ts| (end_ts - round_info.unwrap().start_ts) * 1000.0).map(|x| (x as u64) as f64 / 1000.0),
            flow_id,
            step_idx: 1,
...
fn generate_gantt_charts(records: &[CsvRecord], rounds: &[Round], outdir: &str, t0: f64) -> Result<()> {
    // Group records by round
    let mut round_records: HashMap<usize, Vec<&CsvRecord>> = HashMap::new();
    for record in records {
        round_records.entry(record.round_id).or_default().push(record);
    }
...
fn generate_round_gantt(round: &Round, records: &[&CsvRecord], outdir: &str, t0: f64) -> Result<()> {
    ...
    let round_end_beijing = round.end_ts.map(|end| timestamp_to_beijing_time(end)).unwrap_or_else(|| "未结束".to_string());
        ...
        if let (Some(start), Some(duration)) = (record.start_rel_s, record.duration_s) {
            ...
            let (label, detail_info) = match record.step_type.as_str() {
                "nav" | "navigation" => {
                    ...
                    (format!("F{}-nav", record.flow_id), format!("导航→{}", target_pos))
                }
                "arm" => {
                    let action_label = record.action_label.as_deref().unwrap_or("unknown");
                    let action_code = record.action_code.unwrap_or(0);
                    (format!("F{}-arm-{}", record.flow_id, action_code), format!("机械臂:{}({})", action_label, action_code))
                }
                "head" => {
                    (format!("F{}-head", record.flow_id), "头部控制".to_string())
                }
                "waist" => {
                    (format!("F{}-waist", record.flow_id), "腰部控制".to_string())
                }
                _ => {
                    let action_label = record.action_label.as_deref().unwrap_or("unknown");
                    (format!("F{}-{}", record.flow_id, record.step_type), action_label.to_string())
                }
            };
            
            chart_data.push((label, detail_info, relative_start.max(0.0), duration.max(0.0), record.step_type.clone()));
        }
    }
    ...
    let max_time = round_duration.max(chart_data.iter().map(|(_, _, start, dur, _)| start + dur).fold(0.0, f64::max));
    
    let mut chart = ChartBuilder::on(&root)
        .caption(&format!("Round {} Timeline (Total: {:.3}s)\n北京时间: {} - {}", 
                  round.id, round_duration, round_start_beijing, round_end_beijing), 
                 ("sans-serif", 42)) // 保持字体大小，增加时间信息
        .margin(40) // 增加边距
    ...
    for (idx, (_label, detail_info, start, duration, step_type)) in chart_data.iter().enumerate() {
        ...
        chart.draw_series(std::iter::once(Rectangle::new([(*start, y_pos + 0.15), (*start + *duration, y_pos + y_height + 0.15)], color)))?;
        ...
        let font_size = if *duration > 15.0 { 24 } else if *duration > 8.0 { 20 } else if *duration > 2.0 { 16 } else { 14 };
        ...
        if *duration > 1.0 {
            chart.draw_series(std::iter::once(Text::new(
                label_text,
                (text_x, text_y),
                ("sans-serif", font_size).into_font().color(&BLACK).transform(FontTransform::None)
            )))?;
        }
    }
